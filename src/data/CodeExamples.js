...
function App() {
  ...
  const handleAICompletion = async () => {
    const suggestion = await XStackLabs.complete(code);
    setCode(suggestion);
  };
...
export default function Hero() {
  ...
  const handleAISuggestion = async () => {
    const suggestion = await XStackLabs.suggest("hero component");
    return suggestion;
  };
...
export default function Navbar() {
  ...
  const handleSearch = async () => {
    const results = await XStackLabs.search(searchQuery);
    return results;
  };
